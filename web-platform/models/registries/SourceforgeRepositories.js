 ...
 */
/**
 * SourceforgeRepositories.js [Sourceforge packages handler]
 * author: Brian Bleakley on 01/14/2016.
 * updated by: Vincent Kyi <vincekyi@gmail.com>
 *
 * A subclass of Retriever used to extract and update tools from Sourceforge.
 */
...
/**
 * SourceforgeRepositories [Model for retrieving and updating MySQL database for Sourceforge]
 * @constructor
 */
var SourceforgeRepositories = function() {
    var self = this;
    // call Retriever constructor with sourceforge
    Retriever.call(this, "sourceforge");

    // member variables
    this.BASE_URL = 'http://sourceforge.net/directory/science-engineering/';
...
/**
 * SourceforgeRepositories.r_etrieve
 ...
 * @param  {Object} self [Reference to SourceforgeRepositories object]
 * @param  {Function} callback [ A callback function that takes in 2 paramters:
 *                             		1 An error object
 *                               	2 The result message (string or object) ]
 * @return {Function}            [The callback function]
 */
SourceforgeRepositories.prototype._retrieve = function(self, callback) {
    ...
    var searchCategories = function() {
        ...
        request({
                url: self.BASE_URL + category + '/?page=' + pageIndex
            },
            ...
            function(error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    if (raw.external_homepage != null)
                        tool.links = [{
                            "link_name": "Homepage",
                            "link_url": raw.external_homepage
                        }];
                    ...
                    if (restCount == numProjects) {
                        ...
                        fs.writeFile(base.OUTFILE_TEMP_DIRECTORY + outfile, JSON.stringify(aztecEntries, null, 1), function(err) {
                            if (err) {
                                return callback(new Error("Write file error."), null);
                            }
