...
classifier_list = [SVC(gamma=0.1, kernel='rbf', C=10, class_weight='balanced'),
                   MLPClassifier(activation='logistic', algorithm='l-bfgs', alpha=0.001, hidden_layer_sizes=(4, 2),
                                 max_iter=1000),
...
def get_test_set(directory):
    ...
    for file in files:
        ...
        subprocess.call(['pdftotext', file, out_filename])
...
def main(directory, pdf_directory=None):
    ...
    classifier_list[index].fit(x_train, y_train)
    y_pred = classifier_list[index].predict(x_test)
    if pdf_directory is None or not os.path.isdir(pdf_directory):
        pdf_directory = 'tools/'
        os.makedirs(pdf_directory)

    for num, pub in zip(y_pred, test_files):
        if num:
            os.rename(directory/pub, pdf_directory/pub)
